...
const CursosCapacitacionesSection = () => {
  ...
  if (loading) {
    return (
      <Box sx={{ py: { xs: 4, md: 8 }, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 300 }}>
        <CircularProgress />
      </Box>
    );
  }
    ...
    <Box sx={{
          ...
          >
            {courses.map((course, index) => (
              <Grid
                ...
                xs={6}
                sm={4}
                md={courses.length <= 3 ? 12 / courses.length : 3}
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ textAlign: 'center', pb: 2 }}>
          <Typography sx={{ mb: 2, color: '#00325a' }}>{selectedCourse?.summary}</Typography>
